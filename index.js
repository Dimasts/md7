...
/** LOCAL MODULE */
const { color, bgColor, msgs, pluginLoader } = require("./utils");
...
global.shortenerAuth =
  process.env.sid_email !== "" && process.env.sid_password !== "";
...
const start = async () => {
  ...
  console.log(
    color("[SYS]", "cyan"),
    `Loaded Plugins`,
    color(Object.keys(plugins).length, "#38ef7d")
  );
      ...
      for (let x of participants) {
        ...
        client.sendMessage(m.chat, { text: caption, footer: `Â© 2020 - @lorddimas_`, templateButtons: btn }, { quoted: m })
          ...
          client.sendMessage(jid, {
            image: { url: dp },
            mentions: [x],
            caption: `Sugeng tindak @${x.split("@")[0]}`,
          });
      ...
      for (let name in plugins) {
        ...
        if (plugin.cmd && plugin.cmd.includes(cmd) && !m.isBot) {
          ...
          if (
            ...
            typeof plugin.botAdmin != "undefined" &&
            plugin.botAdmin &&
            !isBotGroupAdmin &&
            isGroupMsg
          ) {
          ...
          plugin.regex instanceof RegExp &&
          plugin.regex.test(body) &&
          !m.isBot
        ) {
          ...
          plugin.startsWith &&
          body.startsWith(plugin.startsWith) &&
          !m.isBot
        ) {
          if (typeof plugin.owner != "undefined" && plugin.owner && !isOwner)
            return;
          if (
            typeof plugin.admin != "undefined" &&
            plugin.admin &&
          ...
          if (
            ...
            return;
          if (typeof plugin.group != "undefined" && plugin.group && !isGroupMsg)
            return;
          await plugin.exec(m, client, {
            body,
            prefix,
